...
// Это позволяет читать без блокировок и без "разорванных" кадров
//
...
namespace cast::ipc {
    ...
    // Подняли до 4K: если разрешение игры больше этого лимита, кадр обрезается,
    // и UI растягивается со сдвигом координат (клики промахиваются). 3840*2160*4
    // ≈ 33 МБ на отображение — приемлемо
...
#pragma pack(push, 4)
    struct FrameHeader {
        ...
        uint32_t     reserved[2];
        // далее в памяти идут пиксели: height * pitch байт (BGRA)
    };
#pragma pack(pop)
